 ...
 * 时    间：2019/12/7
 */
public class ImageRenderer extends  BackgroundRender implements GLSurfaceView.Renderer {
    ...
    public ImageRenderer(Context context) {
        mContext = context;
        try {
            mBitmap = BitmapFactory.decodeStream(mContext.getResources().getAssets().open("eight.jpg"));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public int createOpenGLProgram(String vertexSource, String fragmentSource) {
        ...
        if (vertex == 0) {
            Log.e("dimen", "loadShader vertex failed");
            return 0;
        }
        int fragment = loadShader(GLES20.GL_FRAGMENT_SHADER, fragmentSource);
        if (fragment == 0) {
            Log.e("dimen", "loadShader fragment failed");
            return 0;
        }
        ...
        if (program != 0) {
            ...
            if (linkStatus[0] != GLES20.GL_TRUE) {
                Log.e("dimen", "Could not link program:" + GLES20.glGetProgramInfoLog(program));
